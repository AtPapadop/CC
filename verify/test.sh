...
if [ "$#" -ne 3 ]; then
    echo "Usage: $0 <input_matrix.mtx/mat> <number_of_runs> <output_folder>"
    exit 1
fi
...
./bin/cc_test $INPUT_MATRIX --algorithm bfs --runs $RUNS -o $OUTPUT_FOLDER
echo ""
...
do
    ...
    ./bin/cc_test $INPUT_MATRIX --threads $t --runs $RUNS -o $OUTPUT_FOLDER
    echo ""
done
...
do
    ...
    CILK_NWORKERS=$t ./bin/cc_test_cilk $INPUT_MATRIX --runs $RUNS -o $OUTPUT_FOLDER
    echo ""
done
...
do
    ...
    ./bin/cc_test_pthreads $INPUT_MATRIX --threads $t --runs $RUNS -o $OUTPUT_FOLDER
    echo ""
done
